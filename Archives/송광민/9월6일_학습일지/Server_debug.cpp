#define _CRT_SECURE_NO_WARNINGS // 구형 c 함수 사용 시 경고를 표시하지 않음
#define _WINSOCK_DEPRECATED_NO_WARNINGS // 구형 소켓 api 사용 시 경고를 표시하지 않음

#include <winsock2.h> // 윈속 2 메인 헤더
#include <ws2tcpip.h> // 윈속 2 확장 헤더
...
// 소켓 함수 호출 실패 시 오류 메시지를 표시하고 프로그램을 종료하는 함수
...
/*** 여기까지가 모든 예제에서 사용하는 코드. ***/
/*** 2장 이후에서는 Common.h를 포함하는 방식으로 이 코드 쓸거다.  ***/
...
DWORD WINAPI TCPServer4(LPVOID arg)
{
	...
	while (1) {
		...
		while (1) {
			...
			char* sendbuf;
			sendbuf = (char*)malloc(retval);
			sendbuf[0] = retval + '0';
			send(client_sock, sendbuf, retval, 0);
